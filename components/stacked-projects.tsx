    ...
    <section className="relative w-full bg-neutral-950 py-24 px-4 sm:px-8">
      <div className="mx-auto max-w-5xl space-y-12">
        <div className="text-center space-y-3">
          <span className="text-xs font-mono uppercase tracking-widest text-cyan-400">02 // PORTFOLIO</span>
...
function ProjectCard({ project, index, total }: { project: Project; index: number; total: number }) {
  ...
  return (
    <motion.div
      ...
      className="sticky rounded-3xl border border-white/15 p-8 sm:p-12 shadow-2xl backdrop-blur-2xl transition-all"
    >
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-4 max-w-2xl">
          <span className="inline-block rounded-full bg-white/10 px-3.5 py-1 text-xs font-mono text-cyan-300">
            {project.category}
          </span>
        ...
        <div className="flex items-center gap-3">
          <button className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-black shadow-lg hover:scale-110 active:scale-95 transition">
